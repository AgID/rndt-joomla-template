	...
	foreach ($list['pages'] as $k => $page)
	{
		/*if (in_array($k, range($range * $step - ($step + 1), $range * $step)))
		{
			if (($k % $step == 0 || $k == $range * $step - ($step + 1)) && $k != $currentPage && $k != $range * $step - $step)
			{
				...
				$page['data'] = str_replace('pagenav', 'pagenav noborder', $page['data']);
			}
		}*/
...
function pagination_item_active(&$item)
{
	...
	if ($item->text == JText::_('JNEXT'))
	{
		...
		$display = '<img src="'.JUri::base().'/templates/rndt/images/icons/Next.png" alt="'.JText::_("JNEXT").'" />';
		//$class2   = 'noborder';
	}
	...
	return '<li' . $class . '><a title="' . $item->text . '" href="' . $item->link . '" class="pagenav '.$class2.'">' . $display . '</a></li>';
}
...
function pagination_item_inactive(&$item)
{
	...
	{
		return '<li class="active bgreen hidden-phone"><a>' . $item->text . '</a></li>';
	}
